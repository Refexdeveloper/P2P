...
export function ensureNavigation(
  ...
  if (role === 'HOD Approver') {
    if (!navigation?.length) {
      merged = getDefaultNavigationForRole('HOD Approver');
    }
  ...
  // L2 Manager: honor admin-selected menus; prefer L2 My Tasks over generic nav.tasks
  if (role === 'PR Manager') {
    if (!navigation?.length) {
      merged = getDefaultNavigationForRole('PR Manager');
    }
    const hasL2Tasks = merged.some((n) => n.code === 'nav.pr_manager_dashboard');
    if (hasL2Tasks) {
      merged = merged.filter((n) => n.code !== 'nav.tasks');
    }
    const order = [
