...
import { accountsApi, cloudSubscriptionApi } from '../../../services/api';
...
export default function CloudSubscriptionPanel({
  prId,
  sassInvoice = null,
}: {
  prId: number;
  sassInvoice?: SassInvoiceInfo;
}) {
  ...
  const openInvoice = async () => {
    if (!sassInvoice?.id || !sassInvoice.hasFile) return;
    setOpening(true);
    setFileError('');
    try {
      await accountsApi.downloadInvoiceFile(sassInvoice.id, sassInvoice.fileName || undefined);
    } catch (err) {
      setFileError(err instanceof Error ? err.message : 'Could not open invoice file');
    } finally {
      setOpening(false);
    }
  };
  ...
  if (!sub && !sassInvoice?.hasFile) return null;
        ...
        <div className={`${sub ? 'mt-4 pt-3 border-t border-teal-100' : ''}`}>
          ...
          <div className="flex items-center gap-3 rounded-lg border border-teal-100 bg-teal-50/50 px-3 py-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              {sassInvoice.invoiceNumber ? (
                <p className="text-[11px] text-gray-500">{sassInvoice.invoiceNumber}</p>
              ) : null}
            </div>
            <button
              type="button"
          ...
          {fileError ? <p className="text-xs text-red-600 mt-1.5">{fileError}</p> : null}
        </div>
      ) : null}
    </div>
  );
